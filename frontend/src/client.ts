...
export function Environment(name: string): BaseURL {
    return `https://${name}-genagent-fmh2.encr.app`
}
...
/**
 * Client is an API client for the genagent-fmh2 Encore application.
 */
...
export namespace agent {
    ...
    export class ServiceClient {
        ...
        public async GetAgent(id: string): Promise<Agent> {
            ...
            const resp = await this.baseClient.callAPI("GET", `/agent/${encodeURIComponent(id)}`)
            return await resp.json() as Agent
        }

        public async SaveAgent(params: Agent): Promise<Agent> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callAPI("POST", `/agent`, JSON.stringify(params))
            return await resp.json() as Agent
        }
    }
}
...
class BaseClient {
    ...
    constructor(baseURL: string, options: ClientOptions) {
        ...
        if (typeof window === "undefined") {
            this.headers["User-Agent"] = "genagent-fmh2-Generated-TS-Client (Encore/v1.45.2)";
        }
