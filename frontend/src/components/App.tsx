...
function App() {
  ...
  useEffect(() => {
    const fetchAgent = async () => {
      // If we do not have an id then we are creating a new agent, nothing needs to be fetched
      if (!queryParamID) {
        setIsLoading(false);
        return;
      }
      try {
        ...
        const response = await client.agent.GetAgent(queryParamID);
        setCoverImage(response.profile_url || "");
        setContent(response.bio || "");
      } catch (err) {
        console.error(err);
      }
      setIsLoading(false);
  ...
  const saveDocument = async () => {
    try {
      // Send POST request to the backend for saving the agent
      const response = await client.agent.SaveAgent({
        bio: content,
        profile_url: coverImage,
        // If we have an id then we are updating an existing agent, otherwise we are creating a new one
      ...
      // We now have saved agent with an ID, we can now show the sharing modal
